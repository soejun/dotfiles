...
update() {
	# Bar
	#
	# We need to make two request (more like three) we need to get latitude and longitude
	# then we need to pass that into the /points endpoint of api.weather.gov
	# then we need to jq that stuff and then pass the output into the POST request
	...
	lat_long=$(curl -s ipinfo.io/loc)
	points_response=$(curl -s https://api.weather.gov/points/$lat_long)
	gridId=$(echo "$points_response" | jq -r '.properties | .gridId')
	gridX=$(echo "$points_response" | jq -r '.properties |  .gridX ')
	gridY=$(echo "$points_response" | jq -r '.properties |  .gridY')
	weather=$(curl -s "https://api.weather.gov/gridpoints/$gridId/$gridX,$gridY/forecast")
	temp=$(echo "$weather" | jq -r '.properties.periods[0].temperature')
	forecast=$(echo "$weather" | jq -r '.properties.periods[0].shortForecast')
	time=$(echo "$weather" | jq -r '.properties.periods[0].isDaytime')
	...
	# popup
	url=$(jq -r '.wttr | "\(.url)\(.location)?\(.format)"' ~/.config/sketchybar/plugins/weather/scripts/weather_config.json)
